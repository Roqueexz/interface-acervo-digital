...
interface FormAtualizarAlunoProps {
    id_aluno: number;
}

function FormAtualizarAluno({ id_aluno }: FormAtualizarAlunoProps) {
    ...
    // Carrega os dados atuais do aluno para preencher o formulário
    useEffect(() => {
        const carregarAluno = async () => {
            try {
                setLoading(true);
                const aluno = await AlunoRequests.obterAlunoPorId(id_aluno);
                if (aluno) {
                    setFormData({
                        ...aluno,
                        celular: aluno.celular ? Utilitario.formatarTelefone(aluno.celular) : ''
                    });
                } else {
    ...
    const handleChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        // Formata o celular usando expressão regular
        if (name === 'celular') {
    ...
    const handleSubmit = async (e: React.SubmitEvent<HTMLFormElement>) => {
        ...
        // Valida se o campo e-mail está em formato válido
        if (!Utilitario.validarEmail(formData.email)) {
            alert("E-mail inválido");
            return;
        }

        // Chama o método que faz a requisição PUT à API
        const resposta = await AlunoRequests.atualizarAluno(id_aluno, formData);
        if (resposta) {
    ...
    if (loading) {
        return (
            <main className="bg-gray-100 flex-1 py-8 sm:py-12 px-4 sm:px-6 lg:px-8 flex items-center justify-center">
                <p className="text-slate-600 text-lg font-medium">Carregando dados do aluno...</p>
            </main>
        );
    }
