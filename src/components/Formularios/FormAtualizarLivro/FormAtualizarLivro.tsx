...
interface FormAtualizarLivroProps {
    id_livro: number;
}

function FormAtualizarLivro({ id_livro }: FormAtualizarLivroProps): JSX.Element {
    ...
    // Carrega os dados atuais do livro para preencher o formulário
    useEffect(() => {
        const carregarLivro = async () => {
            try {
                setLoading(true);
                const livro = await LivroRequests.obterLivroPorId(id_livro);
                if (livro) {
                    setFormData(livro);
                } else {
    ...
    const handleSubmit = async (e: React.SubmitEvent<HTMLFormElement>) => {
        ...
        const resposta = await LivroRequests.atualizarLivro(id_livro, formData);
        if (resposta) {
    ...
    if (loading) {
        return (
            <main className="bg-gray-100 flex-1 py-8 sm:py-12 px-4 sm:px-6 lg:px-8 flex items-center justify-center">
                <p className="text-slate-600 text-lg font-medium">Carregando dados do livro...</p>
            </main>
        );
    }
