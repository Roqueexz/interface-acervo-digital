...
function DetalhesEmprestimo({ id_emprestimo }: DetalhesEmprestimoProps): JSX.Element {
    ...
    return (
        <main className="bg-gray-200 flex-1 py-6 sm:py-10 px-4 overflow-y-auto">
            <Card title={`ID: ${emprestimo.id_emprestimo}`} className="shadow-lg animate-fade-in transition-all duration-300 w-full max-w-4xl p-4 sm:p-6 md:p-8 mx-auto font-bold text-xl">
                <div className="flex flex-col gap-2">
                    <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2">
                        <span className="text-gray-500 font-medium tracking-tight">ID do Empréstimo</span>
                        <Tag value={emprestimo.id_emprestimo.toString()} severity="info" className="px-3 py-1" />
                    </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6 p-2">
                        ...
                        <div className="flex flex-col gap-4">
                            ...
                            <div className="flex flex-col gap-3 ml-1 border-l-2 border-blue-50 relative pl-4">
                                ...
                                <div className="flex flex-col">
                                    <span className="text-xs uppercase text-gray-400 font-bold tracking-wider">Status do Empréstimo</span>
                                    <Tag
                                ...
                                <div className="flex flex-col">
                                    ...
                                    <Tag
                                        value={emprestimo.data_devolucao != null ? new Date(emprestimo.data_devolucao).toLocaleDateString('pt-BR') : "Não devolvido"}
                                        severity={emprestimo.data_devolucao != null ? "success" : "danger"}
                                        className="w-fit mt-1 rounded-sm"
                                    />
                                </div>
                            </div>
                        ...
                        <div className="flex flex-col gap-4">
                            <h3 className="text-lg font-semibold text-primary-700 flex items-center gap-2">
                                <i className="pi pi-map-marker text-orange-500"></i> Agentes
                            </h3>
                            <div className="flex flex-col gap-3 ml-1 border-l-2 border-orange-50 relative pl-4">
                                ...
                                <div className="flex flex-col">
                                    ...
                                    <span className="text-gray-700 font-medium leading-relaxed">{emprestimo.aluno.id_aluno}</span>
                                </div>
                            </div>
                        </div>
                    </div>
            ...
            <div className="w-full max-w-4xl mx-auto mt-6 sm:mt-8">
                <button
                    className="w-full bg-slate-700 hover:bg-slate-500 text-white px-4 py-3 md:mb-2 rounded-md font-bold transition-all shadow-md active:scale-95 hover:cursor-pointer"
                    onClick={() => navigate(`/atualizar/emprestimo/${emprestimo.id_emprestimo}`)}
                >
                    Editar Empréstimo
                </button>
                <button
                    className="w-full bg-white text-black hover:bg-slate-500 px-4 py-3 rounded-md font-bold transition-all shadow-md active:scale-95 hover:cursor-pointer"
